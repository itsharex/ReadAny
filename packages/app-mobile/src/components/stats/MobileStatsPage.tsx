/**
 * MobileStatsPage — full-screen stats detail page for mobile
 */
...
import { MobileBarChart } from "@/components/stats/MobileBarChart";
import { MobileTrendChart } from "@/components/stats/MobileTrendChart";
import { MobilePeriodBookList } from "@/components/stats/MobilePeriodBookList";
  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        console.error("[MobileStatsPage] Failed to load stats:", err);
      } finally {
        setLoading(false);
      }
    };
    ...
    <div className="flex h-full flex-col bg-background">
      ...
      <div className="flex-1 overflow-y-auto px-4 pb-6">
        ...
        <div className="mt-4 grid grid-cols-2 gap-3">
          <StatCard icon={<BookOpen className="h-4 w-4" />} title={t("profile.booksRead")} value={`${overall?.totalBooks ?? 0}`} unit={t("profile.booksUnit")} />
          <StatCard icon={<Clock className="h-4 w-4" />} title={t("profile.totalTime")} value={formatTime(overall?.totalReadingTime ?? 0)} />
          <StatCard icon={<Flame className="h-4 w-4" />} title={t("profile.streak")} value={`${overall?.currentStreak ?? 0}`} unit={t("profile.daysUnit")} />
          <StatCard icon={<TrendingUp className="h-4 w-4" />} title={t("profile.avgDaily")} value={formatTime(overall?.avgDailyTime ?? 0)} />
        </div>
            ...
            <div className="flex rounded-md border border-border bg-muted p-0.5">
              <button
                className={`rounded px-2 py-0.5 text-[10px] font-medium transition-colors ${
                  chartView === "heatmap" ? "bg-background text-foreground shadow-sm" : "text-muted-foreground"
                }`}
              ...
              <button
                className={`rounded px-2 py-0.5 text-[10px] font-medium transition-colors ${
                  chartView === "bar" ? "bg-background text-foreground shadow-sm" : "text-muted-foreground"
                }`}
          ...
          {chartView === "bar" && (
            <div className="mb-3 flex items-center justify-between">
              <div className="flex rounded-md border border-border bg-muted p-0.5">
                <button
                  className={`rounded px-2 py-0.5 text-[10px] font-medium transition-colors ${
                    chartMode === "week" ? "bg-background text-foreground shadow-sm" : "text-muted-foreground"
                  }`}
                ...
                <button
                  className={`rounded px-2 py-0.5 text-[10px] font-medium transition-colors ${
                    chartMode === "month" ? "bg-background text-foreground shadow-sm" : "text-muted-foreground"
                  }`}
              ...
              <div className="flex items-center gap-0.5">
                <button className="rounded p-1 text-muted-foreground active:bg-accent" onClick={() => navigatePeriod(-1)}>
        ...
        {overall && overall.longestStreak > 0 && (
          <div className="mt-3 flex items-center gap-2.5 rounded-xl bg-card border border-border p-3">
...
function MobileHeatmap({ dailyStats }: { dailyStats: Array<{ date: string; time: number }> }) {
  ...
  const { weeks, monthLabels } = useMemo(() => {
    ...
    for (const s of dailyStats) statsMap.set(s.date, s.time);
    ...
    while (cursor <= today) {
      ...
      if (dow === 0 && currentWeek.length > 0) { weeksArr.push(currentWeek); currentWeek = []; weekIdx++; }
      if (month !== lastMonth) { mLabels.push({ label: cursor.toLocaleDateString("zh-CN", { month: "short" }), col: weekIdx }); lastMonth = month; }
    ...
    <div className="w-full overflow-x-auto">
      <div className="flex">
        {monthLabels.map((m, i) => {
          ...
          return (
            <div key={`${m.label}-${m.col}`} className="text-[10px] text-muted-foreground" style={{ width: `${span * unit}px`, minWidth: `${span * unit}px` }}>
              {span >= 2 ? m.label : ""}
            </div>
          );
        })}
          ...
          <div key={wi} className="flex flex-col" style={{ gap: `${gap}px` }}>
            {week[0] && week[0].dayOfWeek > 0 && wi === 0 &&
              Array.from({ length: week[0].dayOfWeek }).map((_, i) => (
                <div key={`e-${i}`} style={{ height: `${cellSize}px`, width: `${cellSize}px` }} />
              ))}
            {week.map((day) => (
              <div key={day.date} className={cn("rounded-[2px]", getHeatColor(day.time))} style={{ height: `${cellSize}px`, width: `${cellSize}px` }} />
            ))}
          </div>
        ))}
      </div>
